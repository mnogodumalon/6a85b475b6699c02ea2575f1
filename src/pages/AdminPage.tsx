...
import { KursanmeldungViewDialog } from '@/components/dialogs/KursanmeldungViewDialog';
...
const KURSVERWALTUNG_FIELDS = [
  ...
  { key: 'kursleitung', label: 'Kursleitung', type: 'string/text' },
  ...
  { key: 'aktuelle_belegung', label: 'Aktuelle Belegung', type: 'number' },
];
const KURSANMELDUNG_FIELDS = [
  ...
  { key: 'kurs', label: 'Kurs', type: 'applookup/select', targetEntity: 'kursverwaltung', targetAppId: 'KURSVERWALTUNG', displayField: 'kursname' },
  { key: 'kommentar', label: 'Kommentar', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'kursverwaltung', pascal: 'Kursverwaltung' },
  { key: 'kursanmeldung', pascal: 'Kursanmeldung' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('kursverwaltung');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'kursverwaltung': new Set(),
    'kursanmeldung': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'kursverwaltung': return (data as any).kursverwaltung as Kursverwaltung[] ?? [];
      case 'kursanmeldung': return (data as any).kursanmeldung as Kursanmeldung[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'kursanmeldung':
        lists.kursverwaltungList = (data as any).kursverwaltung ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'kursanmeldung' && fieldKey === 'kurs') {
      const match = (lists.kursverwaltungList ?? []).find((r: any) => r.record_id === id);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    const raw: any[] = (() => {
      switch (entity) {
        case 'kursverwaltung': return KURSVERWALTUNG_FIELDS as any[];
        case 'kursanmeldung': return KURSANMELDUNG_FIELDS as any[];
        default: return [];
      }
    })();
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'kursanmeldung': return {
        create: (fields: any) => LivingAppsService.createKursanmeldungEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateKursanmeldungEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteKursanmeldungEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'kursverwaltung' || dialogState?.entity === 'kursverwaltung') && (
        <KursverwaltungDialog
          open={createEntity === 'kursverwaltung' || dialogState?.entity === 'kursverwaltung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'kursverwaltung' ? handleUpdate : (fields: any) => handleCreate('kursverwaltung', fields)}
          defaultValues={dialogState?.entity === 'kursverwaltung' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Kursverwaltung']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Kursverwaltung']}
        />
      )}
      {(createEntity === 'kursanmeldung' || dialogState?.entity === 'kursanmeldung') && (
        <KursanmeldungDialog
          open={createEntity === 'kursanmeldung' || dialogState?.entity === 'kursanmeldung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'kursanmeldung' ? handleUpdate : (fields: any) => handleCreate('kursanmeldung', fields)}
          defaultValues={dialogState?.entity === 'kursanmeldung' ? dialogState.record?.fields : undefined}
          kursverwaltungList={(data as any).kursverwaltung ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['Kursanmeldung']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Kursanmeldung']}
        />
      )}
      {viewState?.entity === 'kursverwaltung' && (
        <KursverwaltungViewDialog
          open={viewState?.entity === 'kursverwaltung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'kursverwaltung', record: r }); }}
        />
      )}
      {viewState?.entity === 'kursanmeldung' && (
        <KursanmeldungViewDialog
          open={viewState?.entity === 'kursanmeldung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'kursanmeldung', record: r }); }}
          kursverwaltungList={(data as any).kursverwaltung ?? []}
        />
      )}
