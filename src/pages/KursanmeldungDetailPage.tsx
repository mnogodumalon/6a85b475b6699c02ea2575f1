...
import { formEnhancements } from '@/config/form-enhancements/Kursanmeldung';
import { evalComputed } from '@/config/form-enhancements/types';
import { t, appLabel, fieldLabel, localeTag, CURRENCY } from '@/i18n';

export default function KursanmeldungDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [record, setRecord] = useState<Kursanmeldung | null>(null);
  const [loading, setLoading] = useState(true);
  const [editing, setEditing] = useState(false);
  const [deleteOpen, setDeleteOpen] = useState(false);
  const [kursverwaltungList, setKursverwaltungList] = useState<Kursverwaltung[]>([]);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [mainData, kursverwaltungData] = await Promise.all([
        LivingAppsService.getKursanmeldung(),
        LivingAppsService.getKursverwaltung(),
      ]);
      setKursverwaltungList(kursverwaltungData);
  ...
  async function handleUpdate(fields: Kursanmeldung['fields']) {
    if (!record) return;
    await LivingAppsService.updateKursanmeldungEntry(record.record_id, fields);
    await loadData();
    setEditing(false);
  }
  ...
  async function handleDelete() {
    if (!record) return;
    await LivingAppsService.deleteKursanmeldungEntry(record.record_id);
    setDeleteOpen(false);
    navigate('/kursanmeldung');
  }

  function getKursverwaltungDisplayName(url?: unknown) {
    if (!url) return '—';
    const refId = extractRecordId(url);
    return kursverwaltungList.find(r => r.record_id === refId)?.fields.kursname ?? '—';
  }
  ...
  if (!record) {
    return (
      <RecordViewEmpty
        title={t('not_found')}
        action={
          <Button variant="ghost" onClick={() => navigate('/kursanmeldung')}>
  ...
  return (
    <RecordView
      onBack={() => navigate('/kursanmeldung')}
      onEdit={() => setEditing(true)}
      backLabel={t('back')}
      editLabel={t('edit_button')}
    >
      <RecordHeader title={record.fields.vorname ?? appLabel('kursanmeldung')} />
      ...
      <RecordSection title={t('details')} cols={2}>
        ...
        <RecordField label={fieldLabel('kursanmeldung', 'kurs')} value={getKursverwaltungDisplayName(record.fields.kurs)} format="text" />
      ...
      <KursanmeldungDialog
        ...
        kursverwaltungList={kursverwaltungList}
      ...
      <ConfirmDialog
        ...
        title={t('delete_entity', { entity: appLabel('kursanmeldung') })}
