...
export default function KursverwaltungPage() {
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getKursverwaltung());
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} ${t('in_system', { entity: appLabel('kursverwaltung') })}`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder={t('search_entity', { entity: appLabel('kursverwaltung') })}
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
            ...
            <TableRow className="border-b border-input">
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('kursname')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('kursverwaltung', 'kursname')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('beschreibung')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('kursverwaltung', 'beschreibung')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('wochentag')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('kursverwaltung', 'wochentag')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('startzeit')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('kursverwaltung', 'startzeit')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('endzeit')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('kursverwaltung', 'endzeit')}
                  {sortKey === 'endzeit' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('kursleitung')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('kursverwaltung', 'kursleitung')}
                  {sortKey === 'kursleitung' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('schwierigkeitsgrad')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('kursverwaltung', 'schwierigkeitsgrad')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('raum')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('kursverwaltung', 'raum')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('max_teilnehmer')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('kursverwaltung', 'max_teilnehmer')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('aktuelle_belegung')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('kursverwaltung', 'aktuelle_belegung')}
                  {sortKey === 'aktuelle_belegung' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/kursverwaltung/${record.record_id}`); }}>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{lookupLabel('kursverwaltung', 'wochentag', record.fields.wochentag?.key) ?? record.fields.wochentag?.label ?? '—'}</span></TableCell>
                <TableCell>{record.fields.startzeit ?? '—'}</TableCell>
                <TableCell>{record.fields.endzeit ?? '—'}</TableCell>
                <TableCell>{record.fields.kursleitung ?? '—'}</TableCell>
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{lookupLabel('kursverwaltung', 'schwierigkeitsgrad', record.fields.schwierigkeitsgrad?.key) ?? record.fields.schwierigkeitsgrad?.label ?? '—'}</span></TableCell>
                <TableCell>{record.fields.raum ?? '—'}</TableCell>
                <TableCell>{record.fields.max_teilnehmer ?? '—'}</TableCell>
                <TableCell>{record.fields.aktuelle_belegung ?? '—'}</TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={11} className="text-center py-16 text-muted-foreground">
                  {search ? t('no_results') : t('no_data_yet', { entity: appLabel('kursverwaltung') })}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
