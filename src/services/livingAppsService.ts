...
import type { Kursverwaltung, Kursanmeldung, CreateKursverwaltung, CreateKursanmeldung } from '@/types/app';
...
export class LivingAppsService {
  ...
  // --- KURSANMELDUNG ---
  static async getKursanmeldung(): Promise<Kursanmeldung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.KURSANMELDUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Kursanmeldung[];
    return hydrateRecords(records, 'kursanmeldung');
  }
  static async getKursanmeldungEntry(id: string): Promise<Kursanmeldung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.KURSANMELDUNG}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Kursanmeldung;
    return hydrateRecords([record], 'kursanmeldung')[0];
  }
  static async createKursanmeldungEntry(fields: CreateKursanmeldung): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.KURSANMELDUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'kursanmeldung') });
    return { ...data, record_id: data.id };
  }
  static async updateKursanmeldungEntry(id: string, fields: Partial<CreateKursanmeldung>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.KURSANMELDUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'kursanmeldung') });
    return { ...data, record_id: data.id };
  }
  static async deleteKursanmeldungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.KURSANMELDUNG}/records/${id}`);
  }
